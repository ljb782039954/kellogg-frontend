    ...
    <>
      <section className="sticky top-16 md:top-20 z-20 bg-white/90 backdrop-blur-sm border-b border-gray-100 shadow-sm">
        <div className={`max-w-7xl mx-auto px-6 py-3 flex items-center gap-4 flex-wrap ${
          categoryControls ? "justify-between" : "justify-end"
        }`}>
          {categoryControls && (
            <div className="flex gap-1 overflow-x-auto" aria-label={translate("博客分类", "Blog categories")}>
          ...
          <div className="flex items-center gap-3 flex-shrink-0">
            ...
            <div className="flex gap-1 p-1 bg-gray-100 rounded-lg" aria-label={translate("列表视图", "Blog view")}>
              <button
                type="button"
                title={translate("网格视图", "Grid view")}
                aria-label={translate("网格视图", "Grid view")}
              ...
              <button
                ...
                aria-label={translate("列表视图", "List view")}
      ...
      <section className="max-w-7xl mx-auto px-6 py-12 relative min-h-[400px]">
        {isLoading && (
          <div className="absolute inset-0 z-10 flex items-center justify-center bg-white/60 backdrop-blur-[2px]">
            <div className="flex flex-col items-center gap-3">
              ...
              <p className="text-sm text-gray-500">{translate("正在加载文章...", "Loading articles...")}</p>
            </div>
          </div>
        )}
        ...
        {blogs.length === 0 ? (
          <div className="text-center py-24 text-gray-400">
            ...
            <p className="text-lg font-medium">{translate("暂无文章", "No articles yet")}</p>
            <p className="text-sm mt-1">{translate("敬请期待精彩内容", "Stay tuned for exciting content")}</p>
          </div>
        ) : (
          <>
            <div className={viewMode === "list" ? "flex flex-col gap-4" : "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6"}>
              {blogs.map((blog) => {
                ...
                const dateText = formatDate(blog.publish_date || blog.created_at);
                    ...
                    <div className="p-5 flex-1">
                      ...
                      <div className="flex items-center justify-between text-xs text-gray-400 border-t border-gray-100 pt-3 mt-auto">
                        <div className="flex items-center gap-2">
                          ...
                          {blog.author && dateText && <span>·</span>}
